...
def load_berlin(image_set, path, shuffle=False):
    image_set = [y.strip() for y in image_set.split(',')]
    assert image_set, "No image_set specified"

    imdbs = []
    for s in image_set:
        imdbs.append(BerlinPictures(s, path, shuffle, is_train=True))
    if len(imdbs) > 1:
        return ConcatDB(imdbs, shuffle)
    else:
        return imdbs[0]
...
def parse_args():
    ...
    parser.add_argument('--dataset', dest='dataset', help='dataset to use (pascal, berlin)',
...
if __name__ == '__main__':
    ...
    if args.dataset == 'pascal':
        ...
        db = load_berlin(args.set, args.root_path, args.shuffle)
        db.save_imglist(args.target, root=args.root_path)
    else:
